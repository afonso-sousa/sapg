...
def processing_function_wrapper(
    ...
    def preprocess_function(examples):
        ...
        if all_dependencies:
            edge_index = []
            edge_type = []
            for i, node in enumerate(chain.from_iterable(examples["dp"])):
                if node["head"] == -1:
                    continue
                edge_index.append(
                    [
            ...
            model_inputs["edge_type"] = edge_type
            # model_inputs["graph"] = Data(edge_index=edge_index, edge_type=edge_type)
...
def main():
    ...
    all_dependencies = []
    if args.with_graph:
        all_nodes = list(chain.from_iterable(chain.from_iterable(raw_datasets["train"]["dp"])))
    ...
    if args.with_graph:
        model = GraphT5ForConditionalGeneration.from_pretrained("t5-small", config=config)
    elif args.model_name_or_path:
